...
- PDF documents are extracted from `data/raw/`.
...
When PostgreSQL is available, the Java runtime persists the extracted PDF corpus and the active expert-filtered index through `CorpusPersistenceService`, so the relational evidence comes from the same PDF-based execution path used by `main`.
